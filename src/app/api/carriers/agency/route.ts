...
// This endpoint fetches carriers that are associated with products in the user's agency
// Used to populate carrier dropdown in the products tab with only relevant carriers
...
export async function GET(request: Request) {
  try {
    ...
    // Since this is an admin-only route, the middleware will handle authentication
    // We need to get the current user's session to determine their agency
    // For now, we'll use a different approach - let's get the user from the session
    ...
    console.error('API Error in carriers/agency:', error)
    return NextResponse.json({ 
      error: 'Internal Server Error',
      detail: 'An unexpected error occurred while fetching agency carriers'
    }, { status: 500 })
  }
} 
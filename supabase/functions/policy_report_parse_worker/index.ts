...
/* ------------ CONFIG ------------ */
const QUEUE_NAME = 'parse_policy_reports_queue'
const DLQ_NAME = 'parse_policy_reports_dlq'           // create this queue in DB or ignore DLQ if you don’t have it yet
const MAX_RETRIES = 5                                  // after this, archive to DLQ
const VISIBILITY_TIMEOUT_SEC = 180                     // how long a message is invisible while processing
const MAX_POLL_SECONDS = 8                            // long-poll window on each read
const POLL_INTERVAL_MS = 100
const BATCH_SIZE = 1                                   // msgs per read
const INVOCATION_BUDGET_MS_DEFAULT = 20000; // ~20s per request
const INVOCATION_ROW_BUDGET = 1200;   // stop after ~1200 rows flushed
...
/** ---- TYPES ---- */
type Job = {
  path: string         // storage path (e.g., "amam/2025-10/report.csv")
  carrier: string      // e.g., "American Amicable / Occidental"
  agency_id: string
  priority?: number
  enqueued_at?: string
}
...
/* ------------ UTIL RPC WRAPPERS ------------ */
// These RPCs must exist. You already have pgmq_read_with_poll and pgmq_archive.
// If you also exposed a pgmq_send (or pgmq_send_json) RPC, we’ll use it for the DLQ.
// If not, we’ll just archive and log.

async function pgmqReadWithPoll(): Promise<PgmqMessage[]> {
  const { data, error } = await sb.rpc<PgmqMessage[]>('pgmq_read_with_poll', {
    p_queue_name: QUEUE_NAME,
    p_max_msgs: BATCH_SIZE,
    p_visibility_timeout: VISIBILITY_TIMEOUT_SEC,
    p_max_poll_seconds: MAX_POLL_SECONDS,
    p_poll_interval_ms: POLL_INTERVAL_MS
  })
  if (error) throw error
  return data ?? []
}

async function pgmqArchive(msgId: number) {
  const { error } = await sb.rpc('pgmq_archive', { p_queue_name: QUEUE_NAME, p_msg_id: msgId })
  if (error) throw error
}

async function pgmqDelete(msgId: number) {
  const { error } = await sb.rpc('pgmq_delete', { p_queue_name: QUEUE_NAME, p_msg_id: msgId })
  if (error) throw error
}

// Optional DLQ send. If you don’t have an RPC for this, comment out and we’ll only archive.
async function pgmqSendToDlq(payload: unknown) {
  // Change this to your actual RPC if it differs (e.g., 'pgmq_send_json')
  const { error } = await sb.rpc('pgmq_send', {
    p_queue_name: DLQ_NAME,
    p_message: payload as any,
    p_delay: 0
  })
  if (error) throw error
}
...
async function workUntil(deadlineMs: number) {
  let processed = 0
  let pulled = 0

  while (Date.now() < deadlineMs) {
    const batch = await pgmqReadWithPoll().catch(e => {
      console.error('read_with_poll error', e)
      return []
    })
    ...
    for (const rec of batch) {
      try {
        await processJob(rec.message)
        await pgmqArchive(rec.msg_id)           // success → remove
        processed++
      } catch (e) {
        console.error('job failed:', e)
        // retry via visibility timeout; after too many reads, DLQ + archive
        if ((rec.read_ct ?? 0) + 1 >= MAX_RETRIES) {
          await pgmqSendToDlq({ ...rec.message, error: String(e) }).catch(() => {})
          await pgmqArchive(rec.msg_id).catch(err => console.error('archive after max retries failed:', err))
        }
      }
    }
  }

  return { processed, pulled }
}


/**
 * Uploads parsed policy reports into the policy_report_staging table.
 * Runs inside Supabase Edge Functions (Deno).
 *
 * @param supabase - Supabase client instance (service role)
 * @param policyReports - Array of PolicyReportStaging rows
...
async function uploadPolicyReportsToStaging(
  ...
  try {
    if (!policyReports.length) return { success: true, insertedCount: 0 }
    const { error } = await sb
      .from('policy_report_staging')
      .insert(policyReports as any, { defaultToNull: true }) // ← no .select()
    if (error) {
      console.error('❌ staging insert error:', error)
      return { success: false, error: error.message }
    }
    return { success: true, insertedCount: policyReports.length }
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Unknown staging upload error'
    console.error('❌ staging upload error:', msg)
...
/** ---- Orchestrator INSIDE the worker (download → parse-by-carrier → insert → orchestrate) ---- */
async function processJob(job: Job) {
  // 1) download and read into memory (this is fine for 1.8 MB)
  const dl = await sb.storage.from(POLICY_REPORTS_BUCKET).download(job.path)
  if (dl.error) throw dl.error
  const text = await dl.data?.text()
  if (!text) throw new Error(`No file data: ${job.path}`)

  // 2) carrier gate (AMAM only here)
  const c = job.carrier.toLowerCase()
  const isAMAM = c.includes('american amicable') || c.includes('american_amicable') || c.includes('amam')
  if (!isAMAM) throw new Error(`No parser registered for carrier: ${job.carrier}`)

  // 3) parse with Papa *without* building a giant array; batch + flush
  const BATCH_ROWS = 400 // tweak if needed (200–600 are safe)
  let batch: PolicyReportStaging[] = []
  let validated = false
  let firstHeaders: string[] | null = null

  // small helpers (inline to avoid per-row allocations as much as possible)
  const capWord = (w: string) => (w ? w[0].toUpperCase() + w.slice(1).toLowerCase() : '')
  const capWords = (s: unknown) => {
    const t = (s ?? '').toString().trim()
    return t ? t.split(/\s+/).map(capWord).join(' ') : null
  }
  const pushMapped = (r: AMAMPolicyData) => {
    if (!r || !r.Policy || !r.FirstName || !r.LastName || !r.WritingAgent || !r.AgentName || !r.PolicyDate) return
    const policyDate = convertDateFormat(r.PolicyDate)
    const dobIso = convertDateFormat(r.DOB)
    const issueAge = calculateIssueAge(policyDate, dobIso)
    const clientAddress = buildClientAddress(capWords(r.Address1), capWords(r.Address2), capWords(r.Address3), capWords(r.Address4))

    batch.push({
      client_name: `${capWords(r.FirstName) ?? ''} ${capWords(r.LastName) ?? ''}`.trim() || null,
      policy_number: (r.Policy ?? '').toString().trim() || null,
      writing_agent_number: (r.WritingAgent ?? '').toString().trim() || null,
      agent_name: capWords(r.AgentName),
      status: (r.Status ?? '').toString().trim() || null,
      policy_effective_date: policyDate,
      product: (r.Plan ?? '').toString().trim() || null,
      date_of_birth: dobIso,
      issue_age: issueAge,
      face_value: toNumber(r.Face),
      payment_method: (r.Form ?? '').toString().trim() || null,
      payment_frequency: (r.Mode ?? '').toString().trim() || null,
      payment_cycle_premium: toNumber(r.ModePrem),
      client_address: clientAddress,
      client_phone: standardizePhoneNumber(r.Phone),
      client_email: standardizeEmail(r.Email),
      state: (r.State ?? '').toString().trim() || null,
      zipcode: (r.Zip ?? '').toString().trim() || null,
      annual_premium: null,
      client_gender: null,
  ...
  const flush = async () => {
    ...
    const res = await uploadPolicyReportsToStaging(batch)
    if (!res.success) throw new Error(res.error || 'staging failed')
    batch.length = 0 // free memory
  }

  await new Promise<void>((resolve, reject) => {
    Papa.parse<AMAMPolicyData>(text, {
      header: true,
      skipEmptyLines: true,
      // fastMode only if you *know* there are no quoted fields; leave off to be safe.
      step: async (row, parser) => {
        try {
          if (!validated) {
            // validate headers once
            const fields = firstHeaders ?? (row?.meta?.fields ?? [])
            firstHeaders = fields as string[]
            const v = validateAMAMCSVStructure(firstHeaders as string[])
            if (!v.isValid) { parser.abort(); reject(new Error(v.error)); return }
            validated = true
          }
          pushMapped(row.data as AMAMPolicyData)
  ...
  // 4) orchestrate after full file
  const { error: orchErr } = await sb.rpc('orchestrate_policy_report_ingest_with_agency_id', {
...
Deno.serve(async (req) => {
  try {
    ...
    const budgetMs = Number(url.searchParams.get('budget_ms') ?? INVOCATION_BUDGET_MS_DEFAULT)
    const deadline = Date.now() + Math.max(1000, Math.min(budgetMs, 55_000)) // cap under typical limits

    const result = await workUntil(deadline)
 ...
 */
/**
 * Validate an uploaded browser File (client/API layer, not worker).
...
/** -------------------------
 * Edge-safe parsing utilities
 * --------------------------
 * Notes:
 * - Prefer returning `null` for missing/invalid values (matches your staging schema).
 * - The worker should validate CSV structure/content, not browser File metadata.
 */

/**
 * AMAM: Validates AMAM CSV structure and checks for required columns.
 * We only verify headers here; file-type/size checks must be done in the upload or enqueue layer.
 */
export function validateAMAMCSVStructure(headers: string[]): { isValid: boolean; error?: string } {
  if (!Array.isArray(headers) || headers.length === 0) {
    return { isValid: false, error: 'No headers found in CSV.' }
  }
  if (!headers.some(h => h.toLowerCase().includes('company'))) {
    return { isValid: false, error: 'Missing Company column – not an AMAM CSV.' }
  }
  const required = [
    'WritingAgent','AgentName','Company','Policy','Status','DOB','PolicyDate','PaidtoDate','RecvDate',
    'LastName','FirstName','MI','Plan','Face','Form','Mode','ModePrem','Address1','Address2',
    'Address3','Address4','State','Zip','Phone','Email','App Date','WrtPct'
  ]
  const lower = headers.map(h => h.toLowerCase())
  const missing = required.filter(col => !lower.includes(col.toLowerCase()))
  if (missing.length > 0) {
    return { isValid: false, error: `Missing columns: ${missing.join(', ')}` }
  }
  return { isValid: true }
}
...
/**
 * AMAM: Formats agent name from AMAM’s conventions into "First [Middle] Last".
 * Examples:
 *   "JAMAICA/ MARY C TEMU" -> "Mary Temu Jamaica"
 *   "CHICKEN T NUGGET"     -> "Chicken Nugget"
 */
export function formatAMAMAgentName(agentName: unknown): string | null {
  const s = (agentName ?? '').toString().trim()
  if (!s) return null

  const cap = (w: string) => (w ? w.charAt(0).toUpperCase() + w.slice(1).toLowerCase() : '')

  if (s.includes('/')) {
    const [lastRaw, firstMidRaw] = s.split('/')
    if (!firstMidRaw) return cap(s) // unexpected format: best-effort
    const last = cap(lastRaw.trim())
    const parts = firstMidRaw.trim().split(/\s+/)
    const first = cap(parts[0] || '')
    const middle = parts.slice(1).filter(p => p.length > 1).map(cap).join(' ')
    // “First Middle Last LastOrg” (keeping last as org/surname, then the slash part as suffix)
    const arr = [first, middle, last].filter(Boolean)
    return arr.join(' ')
  }

  // No slash → collapse to “First Last”, drop single-letter middles
  const parts = s.split(/\s+/).filter(p => p.length > 1)
  return parts.map(cap).join(' ') || null
}

/** Converts "MM/DD/YYYY" → "YYYY-MM-DD". Returns null if invalid. */
export function convertDateFormat(dateString: unknown): string | null {
...
/**
 * Calculates issue age in full years at policy date (birthday-aware).
 * Returns null if inputs are missing/invalid.
 */
export function calculateIssueAge(policyDate: string | null, dob: string | null): number | null {
  if (!policyDate || !dob) return null
  const p = new Date(policyDate)
  const b = new Date(dob)
  if (isNaN(+p) || isNaN(+b)) return null
...
/** Converts currency/number-like strings to number; strips $ and commas. */
...
/** Builds a single-line address from up to 4 parts. Returns null if all empty. */
export function buildClientAddress(a1?: unknown, a2?: unknown, a3?: unknown, a4?: unknown): string | null {
...
/** Standardizes "First [Middle] Last" capitalization; drops empty parts. */
export function standardizeClientName(first: unknown, middle: unknown, last: unknown): string | null {
  const cap = (w: string) => (w ? w.charAt(0).toUpperCase() + w.slice(1).toLowerCase() : '')
  const f = cap((first ?? '').toString().trim())
  const m = cap((middle ?? '').toString().trim())
  const l = cap((last ?? '').toString().trim())
  const arr = [f, m, l].filter(Boolean)
  return arr.length ? arr.join(' ') : null
}

/** Normalizes payment frequency into one of: Monthly, Quarterly, Semi-Annual, Annual; else Title Case original. */
export function normalizePaymentFrequency(mode: unknown): string | null {
  const s = (mode ?? '').toString().trim()
  if (!s) return null
  const t = s.toLowerCase()
  if (t.includes('monthly') || t === 'month') return 'Monthly'
  if (t.includes('quarterly') || t === 'quarter') return 'Quarterly'
  if (t.includes('semi') || t.includes('semi-annual')) return 'Semi-Annual'
  if (t.includes('annual') || t === 'year') return 'Annual'
  return s.charAt(0).toUpperCase() + s.slice(1).toLowerCase()
}

/**
 * AMAM-specific phone cleaning: returns digits-only if present; null otherwise.
 * (Use this if you want AMAM pipeline to store plain digits; format for display later.)
 */
export function processAMAMPhoneNumber(phone: unknown): string | null {
  const s = (phone ?? '').toString()
  const digits = s.replace(/\D/g, '')
  return digits.length ? digits : null
}

/** Generic phone standardizer: digits-only; null if none. */
export function standardizePhoneNumber(phone: unknown): string | null {
  const s = (phone ?? '').toString()
  const digits = s.replace(/\D/g, '')
  return digits.length ? digits : null
}

/** Lower-cases emails; null if empty. */
export function standardizeEmail(email: unknown): string | null {
  const s = (email ?? '').toString().trim()
  return s ? s.toLowerCase() : null
}

/**
 * Cleans CSV cells that sometimes arrive as =("value") from Excel-exported CSVs.
 * Returns trimmed string or null if empty.
 */
export function cleanCSVValue(value: unknown): string | null {
...
/**
 * AMAM: Parses AMAM CSV text → PolicyReportStaging[]
 * NOTE: This function expects CSV content as a string. No download is attempted here.
 */
export function parseAMAMCSVToPolicyReports(
  csvContent: string,
  carrierName: string,
  agencyId: string
): PolicyReportStaging[] {
  ...
  if (!records.length) throw new Error('No data found in CSV file')
  ...
  // 3) Quick AMAM sanity check
  const first = records[0]
  if (!first || !cleanValue(first.Company)) {
    throw new Error("You uploaded the wrong company's policy report to the AMAM section")
  }

  // 4) Map rows → PolicyReportStaging[]
  const out: PolicyReportStaging[] = []
  for (let i = 0; i < records.length; i++) {
    const r = records[i] as AMAMPolicyData
    try {
      const required = ['FirstName','LastName','Policy','WritingAgent','AgentName','Status','PolicyDate'] as const
      const missing = required.filter(k => !cleanValue(r[k]))
      if (missing.length) {
        console.log(`Row ${i + 1} missing required fields: ${missing.join(', ')}`, r)
        continue
      }

      const policyNumber = cleanCSVValue(r.Policy)
      const policyDate   = convertDateFormat(r.PolicyDate)
      const dobIso       = convertDateFormat(r.DOB)
      const issueAge     = calculateIssueAge(policyDate, dobIso)

      const capWord = (w: string) => (w ? w.charAt(0).toUpperCase() + w.slice(1).toLowerCase() : '')
      const capWords = (s: unknown) => {
        const t = (s ?? '').toString().trim()
        return t ? t.split(/\s+/).map(capWord).join(' ') : null
      }

      const firstName = capWords(r.FirstName) ?? ''
      const lastName  = capWords(r.LastName) ?? ''
      const clientName = `${firstName} ${lastName}`.trim()

      const agentName = formatAMAMAgentName(r.AgentName)

      const a1 = capWords(r.Address1) ?? ''
      const a2 = capWords(r.Address2) ?? ''
      const a3 = capWords(r.Address3) ?? ''
      const a4 = capWords(r.Address4) ?? ''
      const clientAddress = buildClientAddress(a1, a2, a3, a4)

      const paymentFrequency = normalizePaymentFrequency(r.Mode)
      const phoneDigits = processAMAMPhoneNumber(r.Phone)
      const emailLc = standardizeEmail(r.Email)

      const faceValue = toNumber(r.Face)
      const modePrem  = toNumber(r.ModePrem)

      out.push({
        client_name: clientName || null,
        policy_number: policyNumber ? policyNumber : null,
        writing_agent_number: cleanValue(r.WritingAgent),
        agent_name: agentName,
        status: cleanValue(r.Status),
        policy_effective_date: policyDate,
        product: cleanValue(r.Plan),
        date_of_birth: dobIso,
        issue_age: issueAge,
        face_value: faceValue,
        payment_method: cleanValue(r.Form),
        payment_frequency: paymentFrequency ? paymentFrequency : null,
        payment_cycle_premium: modePrem,
        client_address: clientAddress,
        client_phone: phoneDigits,
        client_email: emailLc,
        state: cleanValue(r.State),
        zipcode: cleanValue(r.Zip),
        annual_premium: null,
        client_gender: null,
        agency_id: agencyId,
        carrier_name: carrierName || 'American Amicable / Occidental',
      })
    } catch (rowErr) {
      console.error(`Error processing row ${i + 1}:`, rowErr, records[i])
    }
  }
...
//   curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/policy_report_parse_worker' \
